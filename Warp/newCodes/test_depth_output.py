...
def test(args):
    ...
    path_depth1 = args.test_path + 'warp_depth1/'
    if not os.path.exists(path_depth1):
        os.makedirs(path_depth1)
    path_depth2 = args.test_path + 'warp_depth2/'
    if not os.path.exists(path_depth2):
        os.makedirs(path_depth2)
    ...
    for i, batch_value in enumerate(test_loader):
        ...
        with torch.no_grad():
            batch_out = build_depth_output_model(net, inpu1_tesnor, inpu2_tesnor, depthInput1_tensor, depthInput2_tensor)
...
#         print("depth_warp1.shape:{}".format(depth_warp1.shape))
        ...
        depth_warp1 = (depth_warp1).cpu().detach().numpy().transpose(1,2,0)
        depth_warp2 = (depth_warp2).cpu().detach().numpy().transpose(1,2,0)
        ...
        path = path_depth1 + str(i+1).zfill(6) + ".jpg"
        cv2.imwrite(path, depth_warp1)
        path = path_depth2 + str(i+1).zfill(6) + ".jpg"
        cv2.imwrite(path, depth_warp2)
...
if __name__=="__main__":
    ...
    parser.add_argument('--test_path', type=str, default='/root/UDIS2/dataSet/training/')
