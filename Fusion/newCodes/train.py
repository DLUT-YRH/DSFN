...
import glob
from loss import cal_boundary_term, cal_smooth_term_stitch, cal_smooth_term_diff, cal_smooth_term_diff_new, cal_seam_area_loss, cal_depth_diff_loss, cal_smooth_term_stitch_new
...
#         sigma_seam_area_gradient_loss = 0.
        ...
        for i, batch_value in enumerate(train_loader):
            ...
            depth_warp1_tensor = batch_value[4].float()
            depth_warp2_tensor = batch_value[5].float()
            if torch.cuda.is_available():
                warp1_tensor = warp1_tensor.cuda()
                warp2_tensor = warp2_tensor.cuda()
                mask1_tensor = mask1_tensor.cuda()
                mask2_tensor = mask2_tensor.cuda()
                depth_warp1_tensor = depth_warp1_tensor.cuda()
                depth_warp2_tensor = depth_warp2_tensor.cuda()
...
#             print('boundary_loss:{}'.format(boundary_loss))
...
if __name__=="__main__":
    ...
    parser.add_argument('--train_path', type=str, default='/root/UDIS2/dataSet/training')
